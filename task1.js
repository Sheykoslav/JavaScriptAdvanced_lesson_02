class Human {
  ...
  talk(string){
    console.log(`Привіт мене звуть ${this.name}. ${string}`);
  }
...
const ZooApp = (function(){
  const nurse = new Nurse({name:'Sophie',age:18,zodiac:'lion'});
  const librarian = new Librarian({name:'Nastia',age:22,zodiac:'aquarius'});
  const hunter = new Hunter({name:'Pavlo',age:44,zodiac:'libra'});
  const worker = new Worker({name:'Vlad',age:60,zodiac:'scorpion'});
